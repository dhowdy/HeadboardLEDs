...
class HttpServer(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if "/Notify?" in self.path:
            self._set_headers()
            null,notification = self.path.split("?")

            # Add some error handling for chrome looping
            redir = "<html><head><meta http-equiv='refresh' content='3;url=.\' /></head><body><h1>Notification Sent! <br></h1></body></html>"
 #           print(redir)
    ...
    def notify(self, notification):
        ...
        text = gTTS(text=notification, lang='en-us') # See Google TTS API for more Languages (Note: This may do translation Also - Needs Testing)
    ...
    def Cast(self, ip_add):
        ...
        url = "http://"+ip_add+":8090/Notification.mp3"
        print (url)
